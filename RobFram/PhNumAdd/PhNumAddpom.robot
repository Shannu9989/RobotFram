...
PhNumberAddition
    [Arguments]    ${Pnumber}    ${url2}    ${Browser}    ${wma_User}    ${wma_Pass}    ${Pol}       
    Click Element    ${Related_Par_Own}
    Sleep    9
    Click Element    ${PhNum_Tab}
    Sleep    2
    Click Element    ${Ph_Addbtn}
    Input Text    ${PhNum_Field}    ${Pnumber}
    Click Element    ${Ph_Typ}
    Click Element    ${Ph_TypOpt}
    Click Element    ${Save_Btn}
    Sleep    2
    Element Should Contain    ${Green_Bann_Mess}    Phone Number successfully Added
